...
Install the extension from the [VSCode marketplace](https://marketplace.visualstudio.com/items?itemName=rollingcheesewheel.jsmacros-intellisense) or use the `.vsix` installer

## Features

* **Type hints**: copies the declaration files to your current working directory, generates or extends a `tsconfig` to create a TypeScript project.
* **Version manage**: multiple versions of declarations can be downloaded, select one to be in current use.
...
* If there are any type discrepencies either use `// @ts-ignore` or JSDoc (`@type`, `@param`). **You need to absolutely certain when overriding the type!**
```js
/**
 * @type {AnvilInventory}
 */
